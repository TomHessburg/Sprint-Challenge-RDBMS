...
1. Explain the difference between `RDBMS` and `SQL`.
    ...
    SQL is the language... RDBMS are ervices like SQLiteStudio or Oracle that allow you to easily manipulate the tables created with that language

1. Why do tables need a `primary key`?

    so each entry in each table has a unique identifyer
